SELECT   lc.FACILITYID,
         ...
         i.CODE_DATE_MAX,
         eid.CDE_DT,
         case when eid.CDE_DT <= lc.PO_RECEIPT_DTE then 'Y' else 'N' end mfg_date_flg,
         ...
         case when eid.CDE_DT <= lc.PO_RECEIPT_DTE then eid.CDE_DT + (i.SHELF_LIFE - (case when tfc.DISTRESS_DAYS > 0 then tfc.DISTRESS_DAYS else i.DISTRESS_DAYS end)) days else eid.CDE_DT - (case when tfc.DISTRESS_DAYS > 0 then tfc.DISTRESS_DAYS else i.DISTRESS_DAYS end) days end pull_date_new,



case when round((lc.INVENTORY_TURN + lc.INVENTORY_PROMOTION + lc.INVENTORY_FWD_BUY) - ((i.CASES_PER_WEEK/7) * (days(eid.CDE_DT - (case when tfc.DISTRESS_DAYS > 0 then tfc.DISTRESS_DAYS else i.DISTRESS_DAYS end)days) - days(current date - 1 day))) + 1) < 0 
     then 0 
     else round((lc.INVENTORY_TURN + lc.INVENTORY_PROMOTION + lc.INVENTORY_FWD_BUY) - ((i.CASES_PER_WEEK/7) * (days(eid.CDE_DT - (case when tfc.DISTRESS_DAYS > 0 then tfc.DISTRESS_DAYS else i.DISTRESS_DAYS end)days) - days(current date - 1 day))) + 1) end at_risk_qty,

case when round((lc.INVENTORY_TURN + lc.INVENTORY_PROMOTION + lc.INVENTORY_FWD_BUY) - ((i.CASES_PER_WEEK/7) * (days(case when eid.CDE_DT <= lc.PO_RECEIPT_DTE then eid.CDE_DT + (i.SHELF_LIFE - (case when tfc.DISTRESS_DAYS > 0 then tfc.DISTRESS_DAYS else i.DISTRESS_DAYS end)) days else eid.CDE_DT - (case when tfc.DISTRESS_DAYS > 0 then tfc.DISTRESS_DAYS else i.DISTRESS_DAYS end) days end) - days(current date - 1 day))) + 1) < 0 
     then 0 
     else round((lc.INVENTORY_TURN + lc.INVENTORY_PROMOTION + lc.INVENTORY_FWD_BUY) - ((i.CASES_PER_WEEK/7) * (days(case when eid.CDE_DT <= lc.PO_RECEIPT_DTE then eid.CDE_DT + (i.SHELF_LIFE - (case when tfc.DISTRESS_DAYS > 0 then tfc.DISTRESS_DAYS else i.DISTRESS_DAYS end)) days else eid.CDE_DT - (case when tfc.DISTRESS_DAYS > 0 then tfc.DISTRESS_DAYS else i.DISTRESS_DAYS end) days end) - days(current date - 1 day))) + 1) end at_risk_qty_new
