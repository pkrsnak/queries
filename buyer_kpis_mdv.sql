...
--         i.FACILITYID FACILITY_ID,
         ...
         'B' DATA_GRANULARITY,
         'W' TIME_GRANULARITY 
from (SELECT i.dept_cd, i.case_upc_cd, i.buyer_id, i.case_cost_amt, i.item_desc, i.season_cd, case when i.rstr_subst_list_cd in ('O1', 'O2', 'O3', 'O4', 'O5') THEN 1 else 0 end I_O_FLG, i.lead_time_id, i.safety_stock_qty, i.forecast_qty, i.on_hand_qty, i.on_order_qty, i.reserved_qty, i.advanced_order_qty, i.order_point_qty, i.on_order_qty, i.econ_order_qty, i.eff_econ_order_qty moq, i.xdock_on_order_qty, i.deal_qty, i.shelf_life_nbr, i.deal_qty poq, (i.reserved_qty + i.advanced_order_qty + i.order_point_qty + i.deal_qty + i.xdock_on_hand_qty + ((i.lead_time_id / 7)) * i.forecast_qty) MAX_INVENTORY_CASES, (i.reserved_qty + i.advanced_order_qty + i.order_point_qty + i.deal_qty + i.xdock_on_hand_qty + ((i.lead_time_id / 7) * i.forecast_qty)) * i.case_cost_amt MAX_INVENTORY, ((i.on_hand_qty + i.on_order_qty) - (i.order_point_qty + i.deal_qty + i.advanced_order_qty + i.xdock_on_hand_qty + i.xdock_on_order_qty + i.reserved_qty)) - (i.forecast_qty * .5) EXCESS_INVENTORY_CASES_CHK, case when ((i.on_hand_qty) - (i.order_point_qty + i.deal_qty + i.advanced_order_qty + i.xdock_on_hand_qty + i.reserved_qty)) - (i.forecast_qty * .5) > 0 then ((i.on_hand_qty) - (i.order_point_qty + i.deal_qty + i.advanced_order_qty + i.xdock_on_hand_qty + i.reserved_qty)) - (i.forecast_qty * .5) else 0 end EXCESS_INVENTORY_CASES, case when ((i.on_hand_qty) - (i.order_point_qty + i.deal_qty + i.advanced_order_qty + i.xdock_on_hand_qty + i.reserved_qty)) - (i.forecast_qty * .5) > 0 then ((i.on_hand_qty) - (i.order_point_qty + i.deal_qty + i.advanced_order_qty + i.xdock_on_hand_qty + i.reserved_qty)) - (i.forecast_qty * .5) else 0 end * i.case_cost_amt EXCESS_INVENTORY FROM whmgr.mdv_item i) factor
group by 3,5
;
