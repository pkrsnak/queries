...
--         date(current date) - (date_part('dow', date current date) - 0) KPI_DATE,  --need end date, not weekid
         ...
         sum(EARNINGS_TOT) KPI_DATA_VALUE
from (
...
SELECT   FISCAL_WEEK_ID,
         ...
         sum(hly.HRS_QTY) HRS_TOT,
         sum(hly.OVERTIME_HRS_QTY) OVERTIME_HRS_TOT,
         sum((hly.HRS_QTY + hly.OVERTIME_HRS_QTY)) TOTAL_HRS
from
(      
...
FROM     WH_OWNER.DCLBR_WK_LOC dwl
         ...
         inner join WH_OWNER.FISCAL_WEEK fw on dwl.FISCAL_WEEK_ID = fw.FISCAL_WEEK_ID AND fw.FISCAL_WEEK_ID = 202018  --(fw.START_DT >= date(date('#CURRENT_DATE_DB2#') - (date_part('dow', date '#CURRENT_DATE_DB2#')+6)) And fw.END_DT <= date(date(current date) - (date_part('dow', date current date)- 0)))
...
SELECT   202018 fiscal_week_id,
         ...
         sum(sly.HRS_QTY / 2) HRS_TOT,
         sum(sly.OVERTIME_HRS_QTY / 2) OVERTIME_HRS_TOT,
         sum((sly.HRS_QTY + sly.OVERTIME_HRS_QTY) / 2) TOTAL_HRS
from
(      
SELECT   dwl.FISCAL_WEEK_ID,
         case
...
FROM     WH_OWNER.DCLBR_WK_LOC dwl
         ...
         inner join WH_OWNER.PS_HR_DEPT hd on dwl.DEPT_ID = hd.DEPT_ID
         inner join WH_OWNER.PS_HR_JOB hj on dwl.JOB_CD = hj.JOB_CD
         inner join WH_OWNER.FISCAL_WEEK fw on dwl.FISCAL_WEEK_ID = fw.FISCAL_WEEK_ID AND fw.FISCAL_WEEK_ID between 202017 and 202018  --(fw.START_DT >= date(date('#CURRENT_DATE_DB2#') - (date_part('dow', date '#CURRENT_DATE_DB2#')+6)) And fw.END_DT <= date(date(current date) - (date_part('dow', date current date)- 0)))
         left outer join WH_OWNER.PS_HR_EARNINGS he on dwl.EARNINGS_CD = he.EARNINGS_CD
WHERE    (hd.S_GL_DEPT_ID in ('8100', '8105', '8110', '8160', '8500', '8115', '8116', '8117', '8716', '8717')
     AND hd.DEPT_ID in ('0839', '0843', '0851', '0842', '1116')
