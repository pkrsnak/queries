...
SELECT   'distribution' SCORECARD_TYPE,
         ...
         'labor_total_dollars' KPI_TYPE,
         'F' DATA_GRANULARITY,
         'W' TIME_GRANULARITY, 202053 kpi_date,
--         date(current date) - (date_part('dow', date current date) - 0) KPI_DATE,  --need end date, not weekid
         FACILITY_ID KPI_KEY_VALUE,
         sum(EARNINGS_TOT) KPI_DATA_VALUE_LTD, --labor_total_dollars
         sum(TOTAL_HRS) KPI_DATA_VALUE_LTH, --labor_total_hours
...
--         inner join WH_OWNER.PS_HR_JOB hj on dwl.JOB_CD = hj.JOB_CD
         inner join WH_OWNER.FISCAL_WEEK fw on dwl.FISCAL_WEEK_ID = fw.FISCAL_WEEK_ID AND fw.FISCAL_WEEK_ID = 202052 --(fw.START_DT >= date(date('#CURRENT_DATE_DB2#') - (date_part('dow', date '#CURRENT_DATE_DB2#')+6)) And fw.END_DT <= date(date(current date) - (date_part('dow', date current date)- 0)))
...
FROM     WH_OWNER.DCLBR_WK_LOC dwl
         ...
         inner join WH_OWNER.PS_HR_DEPT hd on dwl.DEPT_ID = hd.DEPT_ID
--         inner join WH_OWNER.PS_HR_JOB hj on dwl.JOB_CD = hj.JOB_CD
         inner join WH_OWNER.FISCAL_WEEK fw on dwl.FISCAL_WEEK_ID = fw.FISCAL_WEEK_ID AND fw.FISCAL_WEEK_ID between 202051 and 202052  --(fw.START_DT >= date(date('#CURRENT_DATE_DB2#') - (date_part('dow', date '#CURRENT_DATE_DB2#')+6)) And fw.END_DT <= date(date(current date) - (date_part('dow', date current date)- 0)))
         left outer join WH_OWNER.PS_HR_EARNINGS he on dwl.EARNINGS_CD = he.EARNINGS_CD
WHERE    (hd.S_GL_DEPT_ID in ('8100', '8105', '8110') --, '8160', '8500', '8115', '8116', '8117', '8716', '8717')
