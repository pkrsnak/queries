-----------------------------------------------------------------------------------------------------------------------------------------------------
--extracts for Jeannette, don't use the one down below:
...
FROM     (((CRMADMIN.T_WHSE_DEPT T_WHSE_DEPT 
         ...
         LEFT OUTER JOIN CRMADMIN.T_BICEPS_DEAL_2016 T_WHSE_DEAL on T_WHSE_ITEM.FACILITYID = T_WHSE_DEAL.FACILITYID and T_WHSE_ITEM.ITEM_NBR_CD = T_WHSE_DEAL.ITEM_NBR
WHERE    T_WHSE_DEAL.DATE_DEAL_ARRIVE between '2016-01-03' and '2016-12-31'
AND      T_WHSE_DEAL.PROMO_QTY > '0'
AND      T_WHSE_DIV_XREF.SWAT_ID = '061'
...
FROM     (((CRMADMIN.T_WHSE_DEPT T_WHSE_DEPT 
         ...
         inner join CRMADMIN.T_WHSE_USER T_WHSE_USER on T_WHSE_ITEM.BUYER_NBR = T_WHSE_USER.BUYER_NBR) 
         LEFT OUTER JOIN CRMADMIN.T_BICEPS_DEAL_2017 T_WHSE_DEAL on T_WHSE_ITEM.FACILITYID = T_WHSE_DEAL.FACILITYID and T_WHSE_ITEM.ITEM_NBR_CD = T_WHSE_DEAL.ITEM_NBR
WHERE    T_WHSE_DEAL.DATE_DEAL_ARRIVE between '2018-01-01' and '2018-12-29'
AND      T_WHSE_DEAL.PROMO_QTY > '0'
AND      T_WHSE_DIV_XREF.SWAT_ID in ('061', '')
AND      NOT T_WHSE_USER.BUYER_NBR is null -- FOR FETCH ONLY
;
-----------------------------------------------------------------------------------------------------------------------------------------------------
...
WHERE    (i.FACILITYID, i.MERCH_CLASS) in 
          (select distinct FACILITYID, MERCH_CLASS from CRMADMIN.V_WEB_ITEM_CORE 
           where FACILITYID in ('040') 
            and MERCH_DEPT in ('0010','0011','0013','0040','0041','0045','0060','0090'))
