...
--         istat.ITEM_NBR,
         ...
         sum(case when istat.SALES_AMZ > 1000 then 1 else 0 end) amazon_count,
         sum(case when istat.SALES_DG > 1000 then 1 else 0 end) dg_count,
         sum(case when istat.SALES_CORP > 1000 then 1 else 0 end) corp_count,
         sum(case when istat.SALES_OTHER > 1000 then 1 else 0 end) ind_count
FROM     ( SELECT DATE_PART('YEAR', dsh.TRANSACTION_DATE) fiscal_year, dsh.FACILITY_ID, f.FACILITY_NAME, dsh.SHIP_FACILITY_ID, fs.FACILITY_NAME SHIP_FACILITY_NAME, dsh.WHSE_CMDTY_ID, dsh.COMMODITY_CODE, dsh.WHOLESALE_DEPT_ID, dsh.ITEM_NBR, DATE_PART('YEAR', i.ITEM_ADDED_DATE) item_added_year, sum(dsh.TOTAL_SALES_AMT) sales, sum(case when c.CORPORATION_ID = 634001 then dsh.TOTAL_SALES_AMT else 0 end) sales_amz, sum(case when c.CORPORATION_ID = 910022 then dsh.TOTAL_SALES_AMT else 0 end) sales_dg, sum(case when c.CORPORATION_ID = 3999 then dsh.TOTAL_SALES_AMT else 0 end) sales_corp, sum(case when c.CORPORATION_ID not in (634001, 910022, 3999) then dsh.TOTAL_SALES_AMT else 0 end) sales_other FROM WH_OWNER.DC_SALES_HST dsh 
...
SELECT   year(lh.LAYER_FILE_DTE) fiscal_year,
         ...
         int(lh.ITEM_DEPT) WHOLESALE_DEPT_ID,
