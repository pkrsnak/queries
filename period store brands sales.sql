select case when x.lawson_account = '999710' Then '301000' Else x.lawson_account end as Lawson_Account , d1.COMPANY_WEEK_ID,
...
SUM(CASE WHEN SHD.ORDER_TYPE = 'GB'  THEN SHD.QTY_SOLD ELSE SHD.QTY_SOLD - SHD.QTY_SCRATCHED END)  as qty,
...
left outer join  ETLADMIN.T_TEMP_PALL_CONV P on P.FACILITYID = SHD.FACILITYID and P.ITEM_NBR = SHD.ITEM_NBR_HS
 ...
 where d1.date_key between current date - 42 days and current date       
 ...
 group by case when x.lawson_account = '999710' Then '301000' Else x.lawson_account end, d1.COMPANY_WEEK_ID,
d1.company_period_id, d1.company_year_id, 
shd.facilityid, case when (c.NAME like '%FRILL%' or c.NAME like '%NOFR%')
                     then 'NOFR'
                     when (c.NAME like '%BAG%' or c.NAME like '%BNS%') and c.NAME <> 'QUALITY TRANSP BAG'
