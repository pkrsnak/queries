...
AND      EXCESS_CHECK + CODE_DATE_CHECK + I_O_CHECK > 0
;
...
create or replace view KPIADMIN.V_KPI_PO_EXCEPTIONS as 
SELECT   i.FACILITYID,
         i.ITEM_DEPT,
         pod.PO_NBR,
         pod.DATE_ORDERED,
         ...
         i.MFG_MIN_ORDER_QTY,
         i.PRODUCT_DATING,
         i.MAX_INVENTORY_CASES,
         i.MAX_INVENTORY,
         i.SHELF_LIFE,
         i.DISTRESS_DAYS,
         pod.TURN po_turn,
         pod.PROMOTION po_promo,
         pod.FORWARD_BUY po_fwd_buy,
         ...
         case 
              when i.I_O_FLAG = 1 then 0 
              else case 
                        when ( max(((i.INVENTORY_TOTAL + i.ON_ORDER_TOTAL) - (i.ORDER_POINT + i.IN_PROCESS_REGULAR + i.IN_PROCESS_PROMO + i.POQ_CURR + i.RESERVE_COMMITTED + i.RESERVE_UNCOMMITTED + i.STORAGE_COMMITTED + i.STORAGE_UNCOMMITTED + (ceiling(((case 
                                                                                                                                                                                                                                                                   when i.ORDER_INTERVAL_WEEKS = 0 then 1 
                                                                                                                                                                                                                                                                   else i.ORDER_INTERVAL_WEEKS end) * 7) * (double(nvl(i.CASES_PER_WEEK, 0)) / 7))) + i.INVENTORY_PROMOTION + i.INVENTORY_FWD_BUY + pod.FORWARD_BUY + pod.PROMOTION)) - ceiling(i.CASES_PER_WEEK * .5), 0) - i.MAX_INVENTORY_CASES) > 0 then 1 
                                                                                                                                                                                                                                                                   else 0 
                                                                                                                                                                                                                                                              end 
                   end excess_check,
         case 
              when (case 
                         when nvl(i.CASES_PER_WEEK, 0) = 0 then 0 
                         else (case 
                                    when (floor(((pod.TURN + pod.FORWARD_BUY) - i.IN_PROCESS_REGULAR - i.IN_PROCESS_PROMO - i.RESERVE_COMMITTED - i.RESERVE_UNCOMMITTED - i.STORAGE_COMMITTED - i.STORAGE_UNCOMMITTED - poq_curr) / (double(i.CASES_PER_WEEK) / 7)) > i.PRODUCT_DATING) then 1 
                                    else 0 end) end) = 1 then (case 
                                                                    when MFG_MIN_ORDER_QTY = 0 then 0 
                                                                    else (case 
                                                                               when int((pod.TURN + pod.FORWARD_BUY) / MFG_MIN_ORDER_QTY) <> 1 then 1 
                                                                               else 0 end) end) 
                                                                               else 0 
                                                                          end code_date_check,
         CASE 
              WHEN i.I_O_FLAG = 1 AND pod.PROMOTION > i.RESERVE_COMMITTED + i.RESERVE_UNCOMMITTED + i.STORAGE_COMMITTED + i.STORAGE_UNCOMMITTED AND i.FACILITYID not in ('001', '066', '067') THEN 1 
              ELSE 0 
         END i_o_check
