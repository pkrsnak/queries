...
SELECT   mi.dept_cd,
         ...
         case 
              when mi.forecast_qty = 0 then 0 
                        ...
                        when ((pod.order_qty - (mi.advanced_order_qty + mi.reserved_qty + mi.xdock_on_order_qty + mi.deal_qty)) / mi.forecast_qty) > mi.shelf_life_nbr then case 
                                                                                                                                                                                 when pod.order_qty > mi.moq then 1 
                                                                                                                                                                                 else 0 
                                                                                                                                                                            end 
                        else 0 
                   end 
         ...
         case 
              when mi.i_o_flg = 1 AND (mi.on_hand_qty + mi.on_order_qty - pod.order_qty) - (mi.reserved_qty + mi.advanced_order_qty + mi.xdock_on_order_qty + mi.xdock_on_hand_qty + mi.deal_qty) > 0 THEN 1 
              ELSE 0 
         END i_o_check
...
WHERE  mi.item_status_cd not in ('new') and    pod.input_date = '01-19-2020'
)
where (excess_check + code_date_check + i_o_check) > 0
;