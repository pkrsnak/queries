...
SELECT   'RDET' As RECORD_SOURCE,
         ...
         rlxref.LAWDEPT FINANCIAL_DEPT, gxref.LAWSON_DEPT,gxref.PRODUCT, 
...
FROM     (( select dxref.SWAT_ID SWAT_ID , dxref.PLATFORM_TYPE PLATFORM_TYPE from CRMADMIN.T_WHSE_DIV_XREF dxref where dxref.ENTERPRISE_KEY = 1) dxref 
         ...
         left outer join ( select gxref.FACILITYID FACILITYID , gxref.DEPTID LAWSON_DEPT , gxref.PRODUCT, gxref.WHOL_SALES_CD WHOL_SALES_CD , gxref.TERRITORY_NO TERRITORY_NO from CRMADMIN.T_WHSE_WSC_TN_TO_GLACCT_XREF  gxref) gxref on sls.FACILITYID = gxref.FACILITYID and sls.WHOL_SALES_CD = gxref.WHOL_SALES_CD and sls.TERRITORY_NO = gxref.TERRITORY_NO 
...
AND      sls.BILLING_DATE between '2017-04-02' and '2017-04-08'
AND      sls.USDS_FLG = 'D'
and sls.FACILITYID <> '002' 
--and lxref.LAWSON_ACCOUNT in (225000, 225010, 248000, 331000)
and lxref.LAWSON_ACCOUNT in (31901,31902,37182,50007,89950)
and sls.item_dept not in ('040')
and not (sls.RECORD_ID = '6' and trim(OVRRDE_RTL_CHRGE_CDE) = 'B')
and rlxref.LAWDEPT is not null
GROUP BY sls.FACILITYID, sls.BILLING_DATE, sxref.STORE_ID, sls.INVOICE_NBR, rlxref.LAWDEPT, gxref.LAWSON_DEPT, gxref.PRODUCT
------------------------------------------------------------------------------------------------------
Union ALL
-- ARDA ----------------------------------------------------------------------------------------------
SELECT   
         ...
         0 as  EXT_FREIGHT, gxref.LAWSON_DEPT,gxref.PRODUCT, 
...
GROUP BY sls.FACILITYID, sls.BILLING_DATE,
         sxref.STORE_ID, sls.INVOICE_NBR, lxref.LAWSON_DEPT, gxref.LAWSON_DEPT,gxref.PRODUCT
------------------------------------------------------------------------------------------------------
Union All
-- TAX MW --------------------------------------------------------------------------------------------
SELECT   
...
AND      sls.ITEM_DEPT not in ('040')
GROUP BY sls.FACILITYID, sls.BILLING_DATE,
         sxref.STORE_ID, sls.INVOICE_NBR, rlxref.LAWDEPT,  gxref.LAWSON_DEPT,gxref.PRODUCT
...
SELECT   'MKUP' As RECORD_SOURCE,
         ...
         sum(case when (sls.NO_CHRGE_ITM_CDE = '*') then 1 
                  ...
                  when (dxref.PLATFORM_TYPE = 'SWAT' and sls.QTY_SOLD = 0 and sls.OUT_REASON_CODE = '000') then 0 
                  when (dxref.PLATFORM_TYPE = 'LEGACY' and sls.RECORD_ID = '6') then 1 
                  when (dxref.PLATFORM_TYPE = 'LEGACY' and case when (sls.ORDER_TYPE = 'GB') then sls.QTY_SOLD 
...
and lxref.LAWSON_ACCOUNT in (60751,87102)
and sls.item_dept not in ('040')
and not (sls.RECORD_ID = '6' and trim(OVRRDE_RTL_CHRGE_CDE) = 'B')
and rlxref.LAWDEPT is not null
GROUP BY sls.FACILITYID, sls.BILLING_DATE, sxref.STORE_ID, sls.INVOICE_NBR, rlxref.LAWDEPT,  gxref.LAWSON_DEPT,gxref.PRODUCT
...
SELECT   'FUEL' As RECORD_SOURCE,
         ...
         sum(case when (sls.NO_CHRGE_ITM_CDE = '*') then 1 
                  ...
                  *sls.FUEL_CHRGE_AMT) fuel_surcharge,
        0 as EXT_RETAIL 
FROM     (( select dxref.SWAT_ID SWAT_ID , dxref.PLATFORM_TYPE PLATFORM_TYPE from CRMADMIN.T_WHSE_DIV_XREF dxref where dxref.ENTERPRISE_KEY = 1) dxref 
         inner join CRMADMIN.T_WHSE_CUST cust on dxref.SWAT_ID = cust.FACILITYID) 
         inner join CRMADMIN.V_WHSE_SALES_HISTORY_DTL sls on cust.FACILITYID = sls.FACILITYID and cust.CUSTOMER_NO = sls.CUSTOMER_NO and cust.TERRITORY_NO = sls.TERRITORY_NO 
         ...
         inner join ( select lxref.FACILITYID FACILITYID , lxref.LAWSON_DEPT LAWSON_DEPT , lxref.LAWSON_ACCOUNT, lxref.WHOL_SALES_CD WHOL_SALES_CD , lxref.TERRITORY_NO TERRITORY_NO from CRMADMIN.V_WHSE_LAWSON_ACCT_TO_WSC_TN lxref where lxref.BUSINESS_SEGMENT = '2') lxref on sls.FACILITYID = lxref.FACILITYID and sls.WHOL_SALES_CD = lxref.WHOL_SALES_CD and sls.TERRITORY_NO = lxref.TERRITORY_NO 
         left outer join ( select gxref.FACILITYID FACILITYID , gxref.DEPTID LAWSON_DEPT , gxref.PRODUCT, gxref.WHOL_SALES_CD WHOL_SALES_CD , gxref.TERRITORY_NO TERRITORY_NO from CRMADMIN.T_WHSE_WSC_TN_TO_GLACCT_XREF  gxref) gxref on sls.FACILITYID = gxref.FACILITYID and sls.WHOL_SALES_CD = gxref.WHOL_SALES_CD and sls.TERRITORY_NO = gxref.TERRITORY_NO 
WHERE    (dxref.platform_type = 'LEGACY' and sls.NO_CHRGE_ITM_CDE NOT in ('*'))
--AND      sls.BILLING_DATE = current date - #NOD# days
AND      sls.BILLING_DATE between '2017-04-02' and '2017-04-08'
AND      sls.USDS_FLG = 'D'
and sls.FACILITYID <> '002' 
...
SELECT   'FUEL' As RECORD_SOURCE,
         ...
         sls.BILLING_DATE BILLING_DATE,
         sxref.STORE_ID STORE_ID,
         sls.INVOICE_NBR INVOICE_NBR, gxref.LAWSON_DEPT,gxref.PRODUCT, 
         '101' FINANCIAL_DEPT,
         0 as QTY_SHIPPED,
         0 as UNITS_SHIPPED,
         0 as REFLECT_OI_AMT_EXT,
...
FROM     (( select dxref.SWAT_ID SWAT_ID , dxref.PLATFORM_TYPE PLATFORM_TYPE from CRMADMIN.T_WHSE_DIV_XREF dxref where dxref.ENTERPRISE_KEY = 1) dxref 
         ...
         inner join ( select lxref.FACILITYID FACILITYID , lxref.LAWSON_DEPT LAWSON_DEPT , lxref.LAWSON_ACCOUNT, lxref.WHOL_SALES_CD WHOL_SALES_CD , lxref.TERRITORY_NO TERRITORY_NO from CRMADMIN.V_WHSE_LAWSON_ACCT_TO_WSC_TN lxref where lxref.BUSINESS_SEGMENT = '2') lxref on sls.FACILITYID = lxref.FACILITYID and sls.WHOL_SALES_CD = lxref.WHOL_SALES_CD and sls.TERRITORY_NO = lxref.TERRITORY_NO 
         left outer join ( select gxref.FACILITYID FACILITYID , gxref.DEPTID LAWSON_DEPT , gxref.PRODUCT, gxref.WHOL_SALES_CD WHOL_SALES_CD , gxref.TERRITORY_NO TERRITORY_NO from CRMADMIN.T_WHSE_WSC_TN_TO_GLACCT_XREF  gxref) gxref on sls.FACILITYID = gxref.FACILITYID and sls.WHOL_SALES_CD = gxref.WHOL_SALES_CD and sls.TERRITORY_NO = gxref.TERRITORY_NO 
WHERE    dxref.platform_type = 'SWAT'
--AND      sls.BILLING_DATE = current date - #NOD# days
AND      sls.BILLING_DATE between '2017-04-02' and '2017-04-08'
