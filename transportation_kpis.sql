...
SELECT   SITEID,
         ...
         ARRIVE_TIME,
         DEPART_TIME,
         LAG(DEPART_TIME) OVER (PARTITION BY SITEID, ROUTEID ORDER BY SITEID, ROUTEID, STOP_NUMBER, STOP_NUMBER) PREVIOUS_DEPART_TIME,
         ARRIVE_TIME - LAG(DEPART_TIME) OVER (PARTITION BY SITEID, ROUTEID ORDER BY SITEID, ROUTEID, DEPART_TIME) drive_time
FROM     TRANSPORT.T_TMS_COM_ROUTE_HISTORY
WHERE    ROUTE_END_TIME between '2019-10-06' and '2019-10-12') x
where drive_time <> 0 and PREVIOUS_DEPART_TIME is not null
;
