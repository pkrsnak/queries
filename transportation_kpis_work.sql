...
--         'cube_total' KPI_TYPE,
         ...
         sum(case when crh.CUBE = 0 then crh.ROUTED_CUBE else crh.CUBE end) KPI_DATA_VALUE_TOTAL_CUBE
FROM     TRANSPORT.T_TMS_COM_ROUTE_HISTORY crh 
         inner join CRMADMIN.T_WHSE_DIV_XREF dx on crh.SITEID = dx.SWAT_ID
         left outer join (SELECT ta.ACCT_ENTITY_ID, ta.REG_ENTITY_ID, tac.ACCT_CTGY_CD FROM TRANSPORT.T_TRA_ACT_ACCOUNTS ta inner join TRANSPORT.T_TRA_ACT_ACCOUNTS_CATEGORY tac on tac.REG_ENTITY_ID = ta.REG_ENTITY_ID and tac.ACCT_CTGY_ENTITY_ID = ta.ACCT_CTGY_ENTITY_ID WHERE tac.ACCT_CTGY_CD = 'DG' GROUP BY ta.ACCT_ENTITY_ID, ta.REG_ENTITY_ID, tac.ACCT_CTGY_CD) x on crh.REG_ENTITY_ID = x.REG_ENTITY_ID and crh.ACCT_ENTITY_ID = x.ACCT_ENTITY_ID 
